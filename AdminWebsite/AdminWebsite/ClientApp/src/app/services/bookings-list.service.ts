...
export class BookingsListService {
  ...
  mapBookingsResponse(bookingsResponse:BookingsResponse): BookingsModel {
     let model = new BookingsModel(bookingsResponse.next_cursor);
    ...
    private addRecords(element: BookingsListModel, bookings: Array<BookingsListModel>) {
        let subSet = bookings.findIndex(s => s.BookingsDate.toString() === element.BookingsDate.toString());
        if (subSet > -1) {
            element.BookingsDetails.forEach(item => {
                let record = bookings[subSet].BookingsDetails.find(x => x.HearingId == item.HearingId);
                if (!record) {
  ...
  private mapBookings(bookingsByDate: BookingsByDateResponse): BookingsListModel {
    let model = new BookingsListModel(bookingsByDate.scheduled_date);
