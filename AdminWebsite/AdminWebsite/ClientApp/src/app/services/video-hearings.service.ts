    ...
    mapParticipantResponseToParticipantModel(response: ParticipantResponse[]): ParticipantModel[] {
        ...
        if (response && response.length > 0) {
            response.forEach(p => {
                ...
                participant.is_judge = p.case_role_name === Constants.HearingRoles.Judge || p.hearing_role_code === Constants.HearingRoleCodes.Judge;
                participant.is_staff_member = p.case_role_name === Constants.HearingRoles.StaffMember || p.hearing_role_code === Constants.HearingRoleCodes.StaffMember;
