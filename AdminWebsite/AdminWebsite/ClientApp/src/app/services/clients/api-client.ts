...
export class BHClient {
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                ...
                for (let item of resultData200)
                    result200!.push(HearingAudioRecordingResponse.fromJS(item));
            }
     ...
     * @return Success
     */
    getCvpAudioRecordingLinkWithCaseReference(cloudroom: string, date: string, caseReference: string): Observable<CvpForAudioFileResponse[]> {
        let url_ = this.baseUrl + "/api/audio/{cloudroom}/{date}/{caseReference}";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetCvpAudioRecordingLinkWithCaseReference(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetCvpAudioRecordingLinkWithCaseReference(<any>response_);
                } catch (e) {
    ...
    protected processGetCvpAudioRecordingLinkWithCaseReference(response: HttpResponseBase): Observable<CvpForAudioFileResponse[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
     ...
     * @return Success
     */
    getCvpAudioRecordingLink(cloudroom: string, date: string): Observable<CvpForAudioFileResponse[]> {
        let url_ = this.baseUrl + "/api/audio/{cloudroom}/{date}";
        if (cloudroom === undefined || cloudroom === null)
            throw new Error("The parameter 'cloudroom' must be defined.");
        url_ = url_.replace("{cloudroom}", encodeURIComponent("" + cloudroom));
        if (date === undefined || date === null)
    ...
    protected processGetCvpAudioRecordingLink(response: HttpResponseBase): Observable<CvpForAudioFileResponse[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
