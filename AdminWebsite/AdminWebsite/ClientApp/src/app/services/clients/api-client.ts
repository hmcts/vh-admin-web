...
export class BHClient {
    ...
    getFeatureToggles(): Observable<FeatureToggleConfiguration> {
        ...
        return this.http
            ...
            .pipe(
                _observableMergeMap((response_: any) => {
                    return this.processGetFeatureToggles(response_);
                })
            )
            .pipe(
                _observableCatch((response_: any) => {
                    if (response_ instanceof HttpResponseBase) {
                        try {
                            return this.processGetFeatureToggles(<any>response_);
                        } catch (e) {
                            return <Observable<FeatureToggleConfiguration>>(<any>_observableThrow(e));
                        }
                    } else return <Observable<FeatureToggleConfiguration>>(<any>_observableThrow(response_));
                })
            );
    }

    protected processGetFeatureToggles(response: HttpResponseBase): Observable<FeatureToggleConfiguration> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body : (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            return blobToText(responseBlob).pipe(
                _observableMergeMap(_responseText => {
                    let result200: any = null;
                    let resultData200 = _responseText === '' ? null : JSON.parse(_responseText, this.jsonParseReviver);
                    result200 = FeatureToggleConfiguration.fromJS(resultData200);
                    return _observableOf(result200);
                })
            );
        } else if (status === 400) {
        ...
        return _observableOf<FeatureToggleConfiguration>(<any>null);
    }
...
export class FeatureToggleConfiguration implements IFeatureToggleConfiguration {
    staff_member?: boolean;

    constructor(data?: IFeatureToggleConfiguration) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property)) (<any>this)[property] = (<any>data)[property];
            }
    ...
    init(_data?: any) {
        if (_data) {
            this.staff_member = _data['staff_member'];
        }
    }

    static fromJS(data: any): FeatureToggleConfiguration {
        data = typeof data === 'object' ? data : {};
        let result = new FeatureToggleConfiguration();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        data['staff_member'] = this.staff_member;
        return data;
    }
}

export interface IFeatureToggleConfiguration {
    staff_member?: boolean;
}
